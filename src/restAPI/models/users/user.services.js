...
async function authenticate({ username, password }){
	...
	if(user && Bcrypt.compareSync(password, user.hash)){
		...
		let { hash, ...userWithoutHash } = user.toObject();
		let token = JWT.sign({sub : user.id }, CONFIG.Database.secret, { expiresIn : '30m'});
...
async function create(userParam){
	...
	if(user && Bcrypt.compareSync(userParam.password, user.hash)){
		let { hash, ...userWithoutHash } = user.toObject();
		let token = JWT.sign({sub : user.id }, CONFIG.Database.secret);
		return { ...userWithoutHash, token };
	}
